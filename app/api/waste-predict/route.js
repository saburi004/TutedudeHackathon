...
//   return prompt;
// }
// app/api/predict/route.js (GET API using Groq)
import { NextResponse } from "next/server";
import dbConnect from "@/lib/dbConnect";
import SalesInput from "@/models/SalesInput";
// const SalesInput = require('../../../models/SalesInput');
...
// Initialize Groq
const groq = new Groq({
  apiKey: process.env.GROQ_API_KEY, // Secure API key usage
});
...
export async function GET(req) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Failed to generate prediction from Groq" },
      { status: 500 }
    );
  }
}

// Prompt generator (kept same name for compatibility)
function generateGeminiPrompt(sales) {
  let prompt = `You are a food waste management AI assistant.
